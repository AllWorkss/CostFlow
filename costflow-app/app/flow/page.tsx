...
export default function FlowPage() {
  ...
  // Build nodes from store blocks
  const enabledBlocks = store.blocks.filter(b => b.enabled);
  ...
  const initialNodes: Node[] = [
    ...
    ...enabledBlocks.map((block, i) =>
      makeNode(
        `block_${block.id}`, `${block.label}`, block.formula,
        560, i * 90, "block", block.result, block.id
      )
    ),
          ...
          {selectedNode && (
            ...
            >
              <div className="flex items-start justify-between mb-4">
                ...
                <button onClick={() => setSelectedNode(null)} className="p-1 rounded-lg" style={{ color: textSec }}>
                  <X size={16} />
                </button>
              </div>
              ...
              <div className="space-y-3">
                <div>
                  <div className="text-xs font-semibold mb-1" style={{ color: textSec }}>Formula</div>
                ...
                {selectedNode.data.value !== undefined && (
                  <div>
                    <div className="text-xs font-semibold mb-1" style={{ color: textSec }}>Computed Value</div>
                    <div className="font-bold text-xl gradient-text">
                      ₹{((selectedNode.data.value as number) ?? 0).toLocaleString("en-IN", { minimumFractionDigits: 2 })}
                    </div>
                  </div>
                )}
              </div>
